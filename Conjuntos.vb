...
Public Class Conjuntos
    ...
    Private Sub Carregar_Conjunto()
        ...
        Try
            ...
            adapter = New MySqlDataAdapter("SELECT conj.id_conjunto, conj.nome, conj.id_status, stat.nome FROM tb_conjuntos AS conj LEFT JOIN tb_status AS stat ON conj.id_status = stat.id_status ORDER BY conj.nome", strConexao)
    ...
    Private Sub Formatar_Datagrid()
        ...
        dtg_Itens.Columns("nome").HeaderText = "Conjunto"
    ...
    Private Sub mnsi_Excluir_Click(sender As Object, e As EventArgs) Handles mnsi_Excluir.Click
        ...
        Try
            Abrir_Conexao()
            sqlcmd = New MySqlCommand("sp_excluir_conjunto", strConexao)
            sqlcmd.CommandType = CommandType.StoredProcedure
            sqlcmd.Parameters.AddWithValue("p_id_conjunto", CONVERT_TO_INTEGER(txt_id_conjunto.Text))
            ...
            If Mensagem_Conclusao = "Valor não existe" Then
                MessageBox.Show("O conjunto '" & txt_Nome.Text & "' não existe na database", "Exclusão não realizada", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Else
                MessageBox.Show("Conjunto '" & txt_Nome.Text & "' excluído com sucesso!", "Exclusão realizada", MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If
    ...
    Private Sub btn_Cancelar_Click(sender As Object, e As EventArgs) Handles btn_Cancelar.Click
        ...
        Else
            ...
            If String.IsNullOrEmpty(GET_DATA_MYSQL("tb_itens_propostas", "id_conjunto", "id_conjunto = " & txt_id_conjunto.Text, Limite:="1")) Then
                mnsi_Excluir.Enabled = True
                mnsi_Excluir.ToolTipText = ""
            Else
                mnsi_Excluir.Enabled = False
                mnsi_Excluir.ToolTipText = "Existem itens de propostas cadastrados com esse conjunto. Portanto, não será possível excluí-lo"
            End If
    ...
    Private Sub btn_Salvar_Click(sender As Object, e As EventArgs) Handles btn_Salvar.Click
        ...
        Try
            Abrir_Conexao()
            If btn_Salvar.Tag = "adicionar" Then
                sqlcmd = New MySqlCommand("sp_inserir_conjunto", strConexao)
                sqlcmd.CommandType = CommandType.StoredProcedure
                ...
                If Mensagem_Conclusao = "Valor já existe" Then
                    MessageBox.Show("O conjunto '" & txt_Nome.Text & "' já existe na database", "Inserção não realizada", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Else
                    MessageBox.Show("Conjunto '" & txt_Nome.Text & "' inserido com sucesso!", "Inserção realizada", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            Else
                If txt_id_conjunto.Text = "" Then
                    ...
                    Exit Sub
                End If
                sqlcmd = New MySqlCommand("sp_editar_conjunto", strConexao)
                sqlcmd.CommandType = CommandType.StoredProcedure
                sqlcmd.Parameters.AddWithValue("p_id_conjunto", CONVERT_TO_INTEGER(txt_id_conjunto.Text))
                sqlcmd.Parameters.AddWithValue("p_nome", txt_Nome.Text)
                sqlcmd.Parameters.AddWithValue("p_id_status", cbo_Status.SelectedValue)
                ...
                Dim Mensagem_Conclusao As String = sqlcmd.Parameters("p_mensagem").Value.ToString
                If Mensagem_Conclusao = "Valor não existe" Then
                    MessageBox.Show("O conjunto '" & txt_Nome.Text & "' não existe na database", "Edição não realizada", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Else
                    MessageBox.Show("Conjunto '" & txt_Nome.Text & "' editado com sucesso!", "Edição realizada", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            End If
    ...
    Private Sub dtg_Itens_SelectionChanged(sender As Object, e As EventArgs) Handles dtg_Itens.SelectionChanged
        ...
        txt_id_conjunto.Text = dtg_Itens.CurrentRow.Cells("id_conjunto").Value
