...
Public Class Fabricantes
    Private Sub Carregar_Fabricantes()
        ...
        Try
            ...
            adapter = New MySqlDataAdapter("SELECT fabr.id_fabricante, fabr.nome, fabr.id_status, stat.nome FROM tb_fabricantes AS fabr LEFT JOIN tb_status AS stat ON fabr.id_status = stat.id_status ORDER BY fabr.nome", strConexao)
    ...
    Private Sub Formatar_Datagrid()
        ...
        dtg_Itens.Columns("nome").HeaderText = "Fabricante"
    ...
    Private Sub Fabricantes_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub mnsi_Editar_Click(sender As Object, e As EventArgs) Handles mnsi_Editar.Click
        ...
        If txt_id_fabricante.Text = "" Then
            Exit Sub
        End If
    ...
    Private Sub mnsi_Excluir_Click(sender As Object, e As EventArgs) Handles mnsi_Excluir.Click
        ...
        If MessageBox.Show("Tem certeza que deseja excluir o fabricante " & txt_Nome.Text & "? O processo não poderá ser desfeito", "Atenção", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No Then
            Exit Sub
        End If
        ...
        Try
            Abrir_Conexao()
            sqlcmd = New MySqlCommand("sp_excluir_fabricante", strConexao)
            sqlcmd.CommandType = CommandType.StoredProcedure
            sqlcmd.Parameters.AddWithValue("p_id_fabricante", CONVERT_TO_INTEGER(txt_id_fabricante.Text))
            ...
            If Mensagem_Conclusao = "Valor não existe" Then
                MessageBox.Show("O fabricante '" & txt_Nome.Text & "' não existe na database", "Exclusão não realizada", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Else
                MessageBox.Show("Fabricante '" & txt_Nome.Text & "' excluído com sucesso!", "Exclusão realizada", MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If
    ...
    Private Sub btn_Cancelar_Click(sender As Object, e As EventArgs) Handles btn_Cancelar.Click
        ...
        Else
            ...
            If String.IsNullOrEmpty(GET_DATA_MYSQL("tb_propostas", "id_fabricante", "id_fabricante = " & txt_id_fabricante.Text, Limite:="1")) Then
                mnsi_Excluir.Enabled = True
                mnsi_Excluir.ToolTipText = ""
            Else
                mnsi_Excluir.Enabled = False
                mnsi_Excluir.ToolTipText = "Existem propostas cadastradas com esse fabricante. Portanto, não será possível excluí-lo"
            End If
    ...
    Private Sub btn_Salvar_Click(sender As Object, e As EventArgs) Handles btn_Salvar.Click
        ...
        Try
            Abrir_Conexao()
            If btn_Salvar.Tag = "adicionar" Then
                sqlcmd = New MySqlCommand("sp_inserir_fabricante", strConexao)
                sqlcmd.CommandType = CommandType.StoredProcedure
                ...
                If Mensagem_Conclusao = "Valor já existe" Then
                    MessageBox.Show("O fabricante '" & txt_Nome.Text & "' já existe na database", "Inserção não realizada", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Else
                    MessageBox.Show("Fabricante '" & txt_Nome.Text & "' inserido com sucesso!", "Inserção realizada", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            Else
                If txt_id_fabricante.Text = "" Then
                    ...
                    Exit Sub
                End If
                sqlcmd = New MySqlCommand("sp_editar_fabricante", strConexao)
                sqlcmd.CommandType = CommandType.StoredProcedure
                sqlcmd.Parameters.AddWithValue("p_id_fabricante", CONVERT_TO_INTEGER(txt_id_fabricante.Text))
                sqlcmd.Parameters.AddWithValue("p_nome", txt_Nome.Text)
                sqlcmd.Parameters.AddWithValue("p_id_status", cbo_Status.SelectedValue)
                ...
                Dim Mensagem_Conclusao As String = sqlcmd.Parameters("p_mensagem").Value.ToString
                If Mensagem_Conclusao = "Valor não existe" Then
                    MessageBox.Show("O fabricante '" & txt_Nome.Text & "' não existe na database", "Edição não realizada", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Else
                    MessageBox.Show("Fabricante '" & txt_Nome.Text & "' editado com sucesso!", "Edição realizada", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            End If
    ...
    Private Sub dtg_Itens_SelectionChanged(sender As Object, e As EventArgs) Handles dtg_Itens.SelectionChanged
        ...
        txt_id_fabricante.Text = dtg_Itens.CurrentRow.Cells("id_fabricante").Value
