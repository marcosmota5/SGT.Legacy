...
Public Class Especificacoes
    ...
    Private Sub Carregar_Conjunto()
        ...
        Try
            ...
            adapter = New MySqlDataAdapter("SELECT espe.id_especificacao, espe.nome, espe.id_status, espe.id_conjunto, conj.nome, stat.nome FROM tb_especificacoes AS espe
LEFT JOIN tb_conjuntos AS conj ON espe.id_conjunto = conj.id_conjunto 
LEFT JOIN tb_status AS stat ON espe.id_status = stat.id_status ORDER BY espe.nome", strConexao)
    ...
    Private Sub Formatar_Datagrid()
        ...
        dtg_Itens.Columns("nome").HeaderText = "Especificação"
        dtg_Itens.Columns("id_status").HeaderText = "ID do Status"
        dtg_Itens.Columns("id_conjunto").HeaderText = "ID do Conjunto"
        dtg_Itens.Columns("nome1").HeaderText = "Conjunto"
        dtg_Itens.Columns("nome2").HeaderText = "Status"
    ...
    Private Sub Conjunto_Load(sender As Object, e As EventArgs) Handles MyBase.Load

        Carregar_Conjunto()
        Preencher_ComboBox(cbo_Status, "tb_status", "id_status", "nome", "id_status, nome", ordenar_por:="nome")
        Preencher_ComboBox(cbo_Conjunto, "tb_conjuntos", "id_conjunto", "nome", "id_conjunto, nome", ordenar_por:="nome")
        ...
        lbl_Conjunto.Enabled = False
        cbo_Conjunto.Enabled = False
    ...
    Private Sub mnsi_Adicionar_Click(sender As Object, e As EventArgs) Handles mnsi_Adicionar.Click
        ...
        lbl_Conjunto.Enabled = True
        cbo_Conjunto.Enabled = True
    ...
    Private Sub mnsi_Editar_Click(sender As Object, e As EventArgs) Handles mnsi_Editar.Click
        ...
        If txt_id_especificacao.Text = "" Then
            Exit Sub
        End If
    ...
    Private Sub mnsi_Excluir_Click(sender As Object, e As EventArgs) Handles mnsi_Excluir.Click
        ...
        If MessageBox.Show("Tem certeza que deseja excluir o conjunto " & txt_Nome.Text & "? O processo não poderá ser desfeito", "Atenção", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No Then
            Exit Sub
        End If
        ...
        Try
            Abrir_Conexao()
            sqlcmd = New MySqlCommand("sp_excluir_especificacao", strConexao)
            sqlcmd.CommandType = CommandType.StoredProcedure
            sqlcmd.Parameters.AddWithValue("p_id_especificacao", CONVERT_TO_INTEGER(txt_id_especificacao.Text))
            ...
            If Mensagem_Conclusao = "Valor não existe" Then
                MessageBox.Show("A especificação '" & txt_Nome.Text & "' não existe na database", "Exclusão não realizada", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Else
                MessageBox.Show("Especificação '" & txt_Nome.Text & "' excluída com sucesso!", "Exclusão realizada", MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If
    ...
    Private Sub btn_Cancelar_Click(sender As Object, e As EventArgs) Handles btn_Cancelar.Click
        ...
        Else
            ...
            If String.IsNullOrEmpty(GET_DATA_MYSQL("tb_itens_propostas", "id_especificacao", "id_especificacao = " & txt_id_especificacao.Text, Limite:="1")) Then
                mnsi_Excluir.Enabled = True
                mnsi_Excluir.ToolTipText = ""
            Else
                mnsi_Excluir.Enabled = False
                mnsi_Excluir.ToolTipText = "Existem itens de propostas cadastrados com essa especificação. Portanto, não será possível excluí-la"
            End If
    ...
    Private Sub btn_Salvar_Click(sender As Object, e As EventArgs) Handles btn_Salvar.Click
        ...
        If cbo_Conjunto.Text = "" Then
            If strObrigatorios = "" Then
                strObrigatorios = lbl_Conjunto.Text
            Else
                strObrigatorios = strObrigatorios & ", " & lbl_Conjunto.Text
            End If
            lbl_Conjunto.ForeColor = Color.Red
        Else
            lbl_Conjunto.ForeColor = Color.Black
        End If
        ...
        Try
            Abrir_Conexao()
            If btn_Salvar.Tag = "adicionar" Then
                sqlcmd = New MySqlCommand("sp_inserir_especificacao", strConexao)
                sqlcmd.CommandType = CommandType.StoredProcedure
                sqlcmd.Parameters.AddWithValue("p_nome", txt_Nome.Text)
                sqlcmd.Parameters.AddWithValue("p_id_conjunto", cbo_Conjunto.SelectedValue)
                sqlcmd.Parameters.AddWithValue("p_id_status", cbo_Status.SelectedValue)
                sqlcmd.Parameters.Add("p_mensagem", MySqlDbType.VarChar, 255).Direction = 2
                sqlcmd.ExecuteNonQuery()
                Dim Mensagem_Conclusao As String = sqlcmd.Parameters("p_mensagem").Value.ToString
                If Mensagem_Conclusao = "Valor já existe" Then
                    MessageBox.Show("A especificação '" & txt_Nome.Text & "' já existe na database", "Inserção não realizada", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Else
                    MessageBox.Show("Especificação '" & txt_Nome.Text & "' inserida com sucesso!", "Inserção realizada", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            Else
                If txt_id_especificacao.Text = "" Then
                    ...
                    Exit Sub
                End If
                sqlcmd = New MySqlCommand("sp_editar_especificacao", strConexao)
                sqlcmd.CommandType = CommandType.StoredProcedure
                sqlcmd.Parameters.AddWithValue("p_id_especificacao", CONVERT_TO_INTEGER(txt_id_especificacao.Text))
                sqlcmd.Parameters.AddWithValue("p_nome", txt_Nome.Text)
                sqlcmd.Parameters.AddWithValue("p_id_conjunto", cbo_Conjunto.SelectedValue)
                ...
                Dim Mensagem_Conclusao As String = sqlcmd.Parameters("p_mensagem").Value.ToString
                If Mensagem_Conclusao = "Valor não existe" Then
                    MessageBox.Show("A especificação '" & txt_Nome.Text & "' não existe na database", "Edição não realizada", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Else
                    MessageBox.Show("Especificação '" & txt_Nome.Text & "' editada com sucesso!", "Edição realizada", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            End If
    ...
    Private Sub dtg_Itens_SelectionChanged(sender As Object, e As EventArgs) Handles dtg_Itens.SelectionChanged
        ...
        txt_id_especificacao.Text = dtg_Itens.CurrentRow.Cells("id_especificacao").Value
        txt_Nome.Text = dtg_Itens.CurrentRow.Cells("nome").Value
        cbo_Conjunto.SelectedValue = dtg_Itens.CurrentRow.Cells("id_conjunto").Value
