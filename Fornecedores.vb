...
Public Class Fornecedores
    Private Sub Carregar_Fornecedores()
        ...
        Try
            ...
            adapter = New MySqlDataAdapter("SELECT forn.id_fornecedor, forn.nome, forn.id_status, stat.nome FROM tb_fornecedores AS forn LEFT JOIN tb_status AS stat ON forn.id_status = stat.id_status ORDER BY forn.nome", strConexao)
    ...
    Private Sub mnsi_Editar_Click(sender As Object, e As EventArgs) Handles mnsi_Editar.Click
        ...
        If txt_id_fornecedor.Text = "" Then
            Exit Sub
        End If
    ...
    Private Sub mnsi_Excluir_Click(sender As Object, e As EventArgs) Handles mnsi_Excluir.Click
        ...
        If MessageBox.Show("Tem certeza que deseja excluir o fornecedor " & txt_Nome.Text & "? O processo não poderá ser desfeito", "Atenção", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No Then
            Exit Sub
        End If
        ...
        Try
            Abrir_Conexao()
            sqlcmd = New MySqlCommand("sp_excluir_fornecedor", strConexao)
            sqlcmd.CommandType = CommandType.StoredProcedure
            sqlcmd.Parameters.AddWithValue("p_id_fornecedor", CONVERT_TO_INTEGER(txt_id_fornecedor.Text))
            ...
            If Mensagem_Conclusao = "Valor não existe" Then
                MessageBox.Show("O fornecedor '" & txt_Nome.Text & "' não existe na database", "Exclusão não realizada", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Else
                MessageBox.Show("Fornecedor '" & txt_Nome.Text & "' excluído com sucesso!", "Exclusão realizada", MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If
    ...
    Private Sub btn_Cancelar_Click(sender As Object, e As EventArgs) Handles btn_Cancelar.Click
        ...
        Else
            ...
            If String.IsNullOrEmpty(GET_DATA_MYSQL("tb_itens_propostas", "id_fornecedor", "id_fornecedor = " & txt_id_fornecedor.Text, Limite:="1")) Then
                mnsi_Excluir.Enabled = True
                mnsi_Excluir.ToolTipText = ""
            Else
                mnsi_Excluir.Enabled = False
                mnsi_Excluir.ToolTipText = "Existem itens de propostas cadastrados com esse fornecedor. Portanto, não será possível excluí-lo"
            End If
    ...
    Private Sub btn_Salvar_Click(sender As Object, e As EventArgs) Handles btn_Salvar.Click
        ...
        Try
            Abrir_Conexao()
            If btn_Salvar.Tag = "adicionar" Then
                sqlcmd = New MySqlCommand("sp_inserir_fornecedor", strConexao)
                sqlcmd.CommandType = CommandType.StoredProcedure
                ...
                If Mensagem_Conclusao = "Valor já existe" Then
                    MessageBox.Show("O fornecedor '" & txt_Nome.Text & "' já existe na database", "Inserção não realizada", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Else
                    MessageBox.Show("Fornecedor '" & txt_Nome.Text & "' inserido com sucesso!", "Inserção realizada", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            Else
                If txt_id_fornecedor.Text = "" Then
                    ...
                    Exit Sub
                End If
                sqlcmd = New MySqlCommand("sp_editar_fornecedor", strConexao)
                sqlcmd.CommandType = CommandType.StoredProcedure
                sqlcmd.Parameters.AddWithValue("p_id_fornecedor", CONVERT_TO_INTEGER(txt_id_fornecedor.Text))
                sqlcmd.Parameters.AddWithValue("p_nome", txt_Nome.Text)
                sqlcmd.Parameters.AddWithValue("p_id_status", cbo_Status.SelectedValue)
                ...
                Dim Mensagem_Conclusao As String = sqlcmd.Parameters("p_mensagem").Value.ToString
                If Mensagem_Conclusao = "Valor não existe" Then
                    MessageBox.Show("O fornecedor '" & txt_Nome.Text & "' não existe na database", "Edição não realizada", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Else
                    MessageBox.Show("Fornecedor '" & txt_Nome.Text & "' editado com sucesso!", "Edição realizada", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            End If
    ...
    Private Sub dtg_Itens_SelectionChanged(sender As Object, e As EventArgs) Handles dtg_Itens.SelectionChanged
        ...
        txt_id_fornecedor.Text = dtg_Itens.CurrentRow.Cells("id_fornecedor").Value
